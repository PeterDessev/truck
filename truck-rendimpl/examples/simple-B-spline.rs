...
impl MyApp {
    ...
    fn regenerate_surfaces(&mut self) {
        let mut polymesh = MyApp::bspline_to_polymesh(&self.bspline, true, self.subdivisions);
    ...
    fn bspline_to_control_poly(
        input: &truck_modeling::BSplineSurface<Point3>,
        ...
        let points = input.control_points().iter().flatten().cloned().collect();
...
impl App for MyApp {
    async fn init(window: Arc<winit::window::Window>) -> MyApp {
        ...
        // This block is dumb but I don't want to manually
        // convert the homogenous coordinates above into
        // Euclidean coordinates.
        // Convert the NURBBS sphere into a pure B-spline surface (probably an elipsoid)
        let v = v
            .into_iter()
            .map(|v| {
                v.into_iter()
        ...
        let mut polymesh = MyApp::bspline_to_polymesh(&bspline, true, 20);
        let (instance, wireframe) = MyApp::load_mesh(&creator, &mut polymesh);
        scene.add_object(&instance);
        scene.add_object(&wireframe);

        let mut control_polymesh = MyApp::bspline_to_control_poly(&bspline);
